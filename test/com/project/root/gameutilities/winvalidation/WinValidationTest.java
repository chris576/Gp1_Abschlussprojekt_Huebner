...
class WinValidationTest extends TestCase {
    ...
    void checkForPasch() {
        ...
        assert win1.getProfit() == WinValidation.Profit_Types.PASCH;
        assert win1.getPoint() == 100;

        int[] eyes2 = {2, 2, 3, 4};
        WinValidation.Win win2 = WinValidation.checkForPasch(eyes2);
        assert win2.getPoint() == 0;
        assert win2.getProfit() == WinValidation.Profit_Types.NONE;
    }
    ...
    void sumEyes() {
        ...
        assert WinValidation.Profit_Types.NONE == win1.getProfit();
        assert win1.getPoint() == 6;

        int[] eyes2 = {3, 3, 4, 6};
        WinValidation.Win win2 = WinValidation.sumEyes(eyes2);
        assert WinValidation.Profit_Types.NONE == win2.getProfit();
